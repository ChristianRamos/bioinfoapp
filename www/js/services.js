...
.service('upload', ["$http", "$q", function ($http, $q){
	/*-----
		recibe como parametros el archivo, el id del usuario, el tipo de perfil, el tipo de archivo("avatar(foto perfil), fotocomentario, documetosProfesional")
	-----*/
	this.uploadFile = function(file,idusuario,tipousuario,tipofoto){
		var deferred = $q.defer();
		var formData = new FormData();
		formData.append("file", file);
		formData.append("idusuario", idusuario);
		formData.append("tpuser", tipousuario);
		formData.append("tfoto", tipofoto);
		return $http.post("http://lukgo.com/Api2/cargaArchivos.php", formData, {
			headers: {
